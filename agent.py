...
def select_entry_candidate(signals, open_tickers):
    ...
    Rules (in order):
        ...
        3. Filter out contracts exceeding MAX_ASK_PER_CONTRACT
        4. If no candidates remain — wait
        5. If one candidate remains — enter it
    ...
    # ── Step 3: Filter by max ask price ───────────────────────────────
    ...
    # ── Step 5: Multiple candidates — check score gap ─────────────────
    top    = eligible[0]
    second = eligible[1]
...
def main():
    ...
    print(f"  Mode            : Enter all clear QUALIFIED signals, 1 contract each")
